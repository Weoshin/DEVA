...
__all__ = ['MetricsTop','MetricsFGTop']
...
class MetricsFGTop():
    ...
    # 定义一个函数，将标签映射到五个分类
    def map_to_five_classes(self, label):
        if label <= -2.8:
            return 0
        elif -2.8 < label <= -2.6:
            return 1
    ...
    def __eval_mosei_regression(self, y_pred, y_true, exclude_zero=False):
        ...
        print(test_truth)
        # 将预测结果和真实标签映射到五个分类
        pred_classes = np.array([self.map_to_five_classes(p) for p in test_preds])
        truth_classes = np.array([self.map_to_five_classes(t) for t in test_truth])

        # 计算预测结果与真实标签中相同分类的数量
        correct_counts = np.sum(pred_classes == truth_classes)

        # 计算准确率
        acc5 = correct_counts / len(test_preds)
        print('acc-5:{}'.format(acc5))
        # test_preds_a7 = np.clip(test_preds, a_min=-3., a_max=3.)
        # test_truth_a7 = np.clip(test_truth, a_min=-3., a_max=3.)
        # test_preds_a5 = np.clip(test_preds, a_min=-2., a_max=-1.)
        # test_truth_a5 = np.clip(test_truth, a_min=-2., a_max=-1.)
        # test_preds_a3 = np.clip(test_preds, a_min=-1., a_max=1.)
        # test_truth_a3 = np.clip(test_truth, a_min=-1., a_max=1.)
