...
class PreNorm(nn.Module):
    ...
    def forward(self, h_t, h_a, h_v, h_minor):
        h_t = self.norm1(h_t)
        h_a = self.norm2(h_a)
        h_v = self.norm3(h_v)
        h_minor = self.norm4(h_minor)

        return self.fn(h_t, h_a, h_v, h_minor)
...
class TPFLearningLayer(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        super().__init__()
        inner_dim = dim_head *  heads
        project_out = not (heads == 1 and dim_head == dim)
    ...
    def forward(self, h_t, h_a, h_v, h_minor):
        ...
        h_minor_shift = self.to_out(out_ta + out_tv)

        h_minor += h_minor_shift

        return h_minor


class TPFLearningEncoder(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, dropout = 0.):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, TPFLearningLayer(dim, heads = heads, dim_head = dim_head, dropout = dropout))
            ]))

    def forward(self, h_t_list, h_a, h_v, h_minor):
        for i, attn in enumerate(self.layers):
            h_minor = attn[0](h_t_list[i], h_a, h_v, h_minor)
        return h_minor
